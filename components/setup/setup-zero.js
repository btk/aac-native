...
import { StyleSheet, View, SafeAreaView, Dimensions, Image, Text, ScrollView, TouchableOpacity } from 'react-native';
  ...
  render() {
    return (
      <SafeAreaView style={styles.holder}>
        <View>
          <Text style={styles.holderTitle}>{API.UIText("setupZeroHeading", this.state.data)}</Text>
          <Text style={styles.holderContent}>{API.UIText("setupZeroContent", this.state.data)}</Text>
        </View>

        <ScrollView style={{ flexDirection: width > height ? "row" : "column", marginHorizontal: "10%", flexWrap: "wrap" }}>
          ...
          <TouchableOpacity onPress={() => this.choose("pt-br")} style={this.getStyle("pt-br")}>
            <Image source={require("../../assets/flag/pt-br.png")} style={styles.avatarImage} />
            <Text style={styles.langText}>Português (BR)</Text>
          </TouchableOpacity>
        </ScrollView>
        <TouchableOpacity onPress={() => this.props.button(this.state.data, 0)} style={styles.button}>
          <Text style={styles.buttonText}>{API.UIText("setupZeroButton", this.state.data)}</Text>
        </TouchableOpacity>
